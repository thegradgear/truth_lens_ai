...
import { Bot, CheckCircle, AlertTriangle, Clock, Tag, Type, Save } from 'lucide-react';
import { format } from 'date-fns';

interface ArticleCardProps {
  article: Article;
  onSave?: (article: Article) => void; // Optional save handler
  showSaveButton?: boolean;
}

export function ArticleCard({ article, onSave, showSaveButton = false }: ArticleCardProps) {
  ...
  return (
    <Card className="shadow-lg w-full flex flex-col">
      ...
      <CardContent className="flex-grow">
        <div className="prose prose-sm max-w-none dark:prose-invert text-foreground overflow-auto max-h-96 p-1 rounded bg-muted/30">
      ...
      <CardFooter className="flex justify-between items-center border-t pt-4">
        <div className="text-xs text-muted-foreground flex items-center">
          ...
          {format(new Date(articleData.timestamp), "MMM d, yyyy 'at' h:mm a")}
        </div>
        {showSaveButton && onSave && (
          <Button onClick={() => onSave(article)} size="sm" variant="outline">
